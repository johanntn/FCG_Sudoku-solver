...
suite('Unit Tests', () => {
    suite("solver tests", function() {
        ...
        test("Logic handles a puzzle string that is not 81 characters in length", function(done){
            let inValidPuzzle = "1.5..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8...16....926914.37"
            assert.equal(solver.completeSudoku(inValidPuzzle), false);
            done();
        });
        ...
        test("Invalid puzzle strings fail the solver", function(done){
            let inValidPuzzle = "111.5..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8...16....926914.37";
            assert.equal(solver.completeSudoku(inValidPuzzle), false);
            done();
        }); 
        test("Solver returns the expected solution for an incomplete puzzle", function(done){
            assert.equal(solver.completeSudoku("..839.7.575.....964..1.......16.29846.9.312.7..754.....62..5.78.8...3.2...492...1"),"218396745753284196496157832531672984649831257827549613962415378185763429374928561");
            done();
        }); 
    });
