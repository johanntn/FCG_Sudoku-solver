...
suite('Functional Tests', () => {
    ...
    test("Solve a puzzle with invalid characters: POST request to /api/solve",function(done){
        ...
        .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.body.error, "Expected puzzle to be 81 characters long"); //COMMENT?
            done();
        });
    });
    test("Solve a puzzle with incorrect length: POST request to /api/solve", function(done) {
        ...
        .post("/api/solve")
        .send({
            puzzle: "1.5..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8...16....926914.37"}) 
        .end(function(err, res){
            assert.equal(res.status, 200);
    ...
    test("Solve a puzzle that cannot be solved: POST request to /api/solve", function(done) {
        ...
        .post("/api/solve")
        .send({
            puzzle: "115..2.84..63.12.7.2..5.....9..1....8.2.3674.3.7.2..9.47...8...16....926914.37"}) 
        .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.body.error, "Expected puzzle to be 81 characters long"); //comment?
            done();
    ...
    test("Check a puzzle placement with incorrect length: POST request to /api/check",function(done){
        ...
        .post("/api/solve")
        .send({
            puzzle: "1.5..2.84..63.12.7.2..5...9..1....8.2.3.3.7.2..9.47...8..1..16....926914.37.",
            coordinate: "A2",
            value: "3"   
        }) 
        .end(function(err, res){
    ...
    test("Check a puzzle placement with invalid placement coordinate: POST request to /api/check",function(done){
        ...
        .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.body.error, "Invalid coordinate"); //COMMENT?
            done();
        });
    });
    test("Check a puzzle placement with invalid placement value: POST request to /api/check",function(done){
        ...
        .end(function(err, res){
            assert.equal(res.status, 200);
            assert.equal(res.body.error, "Invalid value"); //COMMENT?
            done();
        });
    });
